...
describe("Home", () => {
  it("renders the main marketing surface", () => {
    ...
    expect(
      screen.getByRole("heading", { name: /keep your day moving/i }),
    ).toBeInTheDocument();
    const playLinks = screen.getAllByRole("link", {
      name: /download on google play/i,
    });
    ...
    expect(playLinks[1]).toHaveAttribute(
      "href",
      "https://play.google.com/store/apps/details?id=app.dakotitah.doneloop",
    );
    expect(
      screen.getByText("Built for daily follow-through"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("Done Loop is available for Android on Google Play."),
    ).toBeInTheDocument();
  ...
  it("switches visible copy to Spanish", () => {
    ...
    expect(
      screen.getByRole("heading", { name: /mantén tu día avanzando/i }),
    ).toBeInTheDocument();
    const playLinks = screen.getAllByRole("link", {
      name: /descargar en google play/i,
    });
    expect(playLinks).toHaveLength(2);
    expect(playLinks[0]).toHaveAttribute(
      "href",
    ...
    expect(
      screen.getByText(
        "Done Loop está disponible para Android en Google Play.",
      ),
    ).toBeInTheDocument();
